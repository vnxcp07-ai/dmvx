...
// --- TIER CONFIGURATION ---
// This now controls the color and whether the avatar gets a glow.
const TIERS = {
  starfall: { accent: '#ff0000', avatarGlow: true },
  smite:    { accent: '#ff0099', avatarGlow: true },
  nuke:     { accent: '#FF00B5', avatarGlow: false } // Nuke tier has no avatar glow
};
...
// Helper to tint an image with a specific color
...
async function generateDonationImage(donatorName, receiverName, donatorId, receiverId, amount) {
  ...
  // --- FIX: Tint the Robux icon with the tier color ---
  ...
  // --- FIX: Center the avatars vertically ---
  const avatarY = canvas.height / 2;
  
  function drawAvatar(image, x) {
      if (!image) return;
      ctx.save();
      // --- FIX: Only draw glow if the tier allows it ---
      if (tier.avatarGlow) {
  ...
  // --- FIX: Reposition avatars for better balance ---
  ...
  const nameY = avatarY + avatarRadius + 25; // Adjusted spacing
  ...
  // --- FIX: Draw the newly tinted icon ---
