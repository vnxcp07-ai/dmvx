...
const fs = require('fs'); // <-- IMPORTANT: Added the File System module
...
function ensureFont() { // Can be sync now
    if (fontLoaded) return;
    
    // --- THIS IS THE FIX ---
    // 1. Get the absolute path to the font file
    const fontPath = path.join(process.cwd(), 'Poppins-Bold.ttf');
    // 2. Read the file into a buffer
    const fontBuffer = fs.readFileSync(fontPath);
    // 3. Register the buffer. This is much more reliable.
...
async function generateDonationImage(donatorName, receiverName, donatorId, receiverId, amount) {
  ensureFont(); // Load the font synchronously
  ...
  const [donatorBuf, receiverBuf, robuxImg] = await Promise.all([
    ...
    // This method is generally fine for loadImage, but the font loading was the issue.
  ...
  ctx.drawImage(robuxImg, groupStartX, cardHeight / 2 - 35, iconSize, iconSize);
