...
async function generateDonationImage(donatorName, receiverName, donatorId, receiverId, amount) {
  ...
  function drawAvatar(image, x) { /* This function is unchanged */ }
  drawAvatar(donatorImg, 105);
  drawAvatar(receiverImg, cardWidth - 105);

  ctx.textBaseline = 'middle';

  // --- FIX: Center Group (Icon + Amount) ---
  ...
  // Draw Robux Icon at the calculated start position
  ctx.drawImage(robuxImg, groupStartX, cardHeight / 2 - 35, iconSize, iconSize);

  // Draw Amount Text next to the icon
  ctx.textAlign = 'left';
...
// Re-add the drawAvatar function here
generateDonationImage.toString = generateDonationImage.toString().replace(
    'function drawAvatar(image, x) { /* This function is unchanged */ }',
